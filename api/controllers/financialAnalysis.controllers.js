...
export const financialAnalysis = async (req, reply) => {
  // Validate required fields
  // Accept both JSON and file uploads (multipart)
  let fields = {
  ...
  if (req.isMultipart) {
    ...
    for await (const part of parts) {
      if (part.file) {
        // File upload: parse JSON or text
        let content = "";
        ...
        try {
          ...
          fields[part.fieldname] = content; // fallback to raw text
        }
      } else {
        // Regular field
        try {
  ...
  // Use Gemini by default (can be extended)
  const aiResponse = await runGemini({ model: "gemini-1.5-flash", prompt, context: {} });

  // Parse AI response (assume JSON output)
  let insights = {};
  try {
